...
void request_handler(void* arg) {
    ...
    clnt_write = fdopen(dup(clnt_sock), "w");  // dup: 파일 식별자를 복제(새로운 파일 서술자를 반환, 숫자만 다를 뿐 원래의 서술자, 복제된 서술자 모두 같은 파일을 가리킴)
    fgets(req_line, SMALL_BUF, clnt_read);

    printf("Request DATA: %s \n", req_line);
    // HTTP 통신인지를 확인
    if (strstr(req_line, "HTTP/") == NULL) {    // strstr: 두 문자열을 비교해 문자열이 있는지를 확인
